...
def countHandleWithNumber(myArray, numbers)
	puts "Combien d'handle contiennent un numéro ?"
	result = []
	myArray.each do |handle|
		continue = true
		handle.split('').each do |char|
	...
	return puts result.size
end


def countHandleWithAude(myArray)
...
def countHandleStartWithUpperCase(myArray)
	puts "Combien commencent par une majuscule ?"
	result = []
	myArray.each do |handle|
		if /[[:upper:]]/.match?(handle.split('')[1])
			result << handle
		end
	end
...
def countHandleWithUpperCase(myArray)
...
def countHandleWithDash(myArray)
...
def sortHandleCaseInsensitive(myArray)
	puts "Trie la liste de handle par ordre alphabétique "
	sortedArray =  myArray.sort_by { 
		|word| word.downcase
	}

	return puts sortedArray
end

def arraySize(myArray)
	puts "Combien y a t'il de journalistes dans cet array ? "

	return puts myArray.size
end

arraySize(myArray)
countHandleWithNumber(myArray, numbers)
countHandleWithAude(myArray)
countHandleStartWithUpperCase(myArray)
countHandleWithUpperCase(myArray)
countHandleWithDash(myArray)
# sortHandleCaseInsensitive(myArray)